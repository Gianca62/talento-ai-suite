...
import psycopg2
DATABASE_URL = "postgresql://postgres:Paparucci2024@@db.fhrfqngqxoduinimtynu.supabase.co:5432/postgres" 
...
def get_db_connection():
    try:
        ...
        # Mostra un errore se la connessione fallisce
        st.error(f"Errore di connessione al database: {e}. Controlla il DATABASE_URL.")
        return None
...
with tab1:
    ...
    if clienti_df.empty:
        ...
        # Prepara la lista dei nomi dei clienti dal DataFrame permanente
        clienti_options = clienti_df['nome_azienda'].tolist()
        
        with st.form("form_preventivo"):
            
            numero = st.text_input("Numero Preventivo *")
            
            # NUOVO: Seleziona il nome del cliente dal DataFrame
            cliente_selezionato = st.selectbox("Cliente *", clienti_options)
            
            # CRUCIALE: Trova l'ID del cliente per il database
            cliente_id = clienti_df[clienti_df['nome_azienda'] == cliente_selezionato]['id'].iloc[0]
            ...
            valore_totale = st.number_input("Valore Totale €", min_value=0.0, step=0.01)
            
            # Lo stato iniziale per il database
            stato_iniziale = "BOZZA"
            
            if st.form_submit_button("Crea Preventivo", type="primary"):
                if numero and cliente_selezionato:
                    
                    # NUOVO: Chiama la funzione di salvataggio su database
                    if save_nuovo_preventivo(
                        cliente_id, 
                        numero,  # Usiamo 'numero' come titolo/riferimento
                        valore_totale, 
                        stato_iniziale, 
                        None # Margine è opzionale
                    ):
                        st.success(f"Preventivo '{numero}' salvato in modo permanente!")
                        st.rerun()
                else:
...
with tab2:
    ...
    if not preventivi_df.empty:
        df_filtrato = preventivi_df.copy() # Copia per i filtri
        
        # Filtri
        col1, col2 = st.columns(2)
        with col1:
            stati_filtro = st.multiselect("Filtra per stato:", 
                                         df_filtrato['stato'].unique().tolist(),
                                         default=df_filtrato['stato'].unique().tolist())
        
        df_filtrato = df_filtrato[df_filtrato['stato'].isin(stati_filtro)]
        st.dataframe(df_filtrato, use_container_width=True)
        
        # Cambio stato (Aggiornamento sul DB)
        st.subheader("Cambia Stato Preventivo")
        col1, col2, col3 = st.columns(3)
        
        # Uso l'ID per l'aggiornamento, ma mostro il titolo (numero)
        preventivo_sel_id = st.selectbox(
            "Preventivo:", 
            options=df_filtrato.set_index('titolo')['id'].to_dict(), # Associa titolo all'ID
            format_func=lambda x: df_filtrato[df_filtrato['id'] == x]['titolo'].iloc[0] if not df_filtrato.empty else "Seleziona..."
        )
        
        with col2:
            nuovo_stato = st.selectbox("Nuovo Stato:", ["BOZZA", "INVIATO", "ACCETTATO", "RIFIUTATO", "SCADUTO"])
        with col3:
            # Qui usiamo la NUOVA FUNZIONE di aggiornamento DB
            if st.button("Cambia Stato"):
                # Questa funzione è TASSATIVA per l'aggiornamento
                if save_aggiorna_stato_preventivo(preventivo_sel_id, nuovo_stato):
                    st.success(f"Stato preventivo aggiornato a {nuovo_stato}")
                    st.rerun()
    else:
...
# AMMINISTRAZIONE - NUOVA SEZIONE
    ...
    with tab1:
        ...
        if save_nuova_spesa(
            ...
            st.success("Spesa salvata in modo permanente!")
            st.rerun()
    else:
    ...
    # Filtri (Assumiamo che tu abbia i campi 'categoria' e 'data_spesa' per i filtri)
    col1, col2 = st.columns(2)
    with col1:
